...
export default function ProcessVideoModal({ story, onClose, onProcessComplete }) {
  ...
  const handleSubmit = async () => {
    if (!story.videoLink) {
      setLocalError('No video link found for this story');
      return;
    }
    ...
    if (result?.processedVideoUrl) {
      ...
      setSubmitted(false);
      setLocalError(error || 'Processing failed - check that the backend server is running');
    }
  };
      ...
      <div className="process-modal">
        ...
        <div className="modal-body">
          ...
          {!submitted && !processing && (
            <div style={{ padding: '0.75rem', backgroundColor: 'rgba(59, 130, 246, 0.1)', borderRadius: '8px', border: '1px solid rgba(59, 130, 246, 0.3)', fontSize: '0.85rem', color: 'var(--text)' }}>
              <p style={{ margin: '0 0 0.5rem 0', fontWeight: '500' }}>📌 Video Processing</p>
              <p style={{ margin: '0', fontSize: '0.8rem', color: 'var(--dimmer)' }}>
                This feature requires a Flask backend running at http://localhost:5000. Make sure the video processing server is started before using this feature.
              </p>
            </div>
          )}
